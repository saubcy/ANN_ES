...
ANNpointArray annAllocPts_ES(
		ES_INFO*   es_info,
		int n, int dim
		)						// allocate n pts in dim
{
	ANNpointArray pa = new ANNpoint[n];			// allocate points
	es_info->m_value = new ANNcoord[dim];
	es_info->m_dim = dim;
	for (int i = 0; i < n; i++) {
		pa[i] = &(es_info->m_value[0]);
	}
	return pa;
}
...
int	ANNmaxPtsVisited[20];	// maximum number of pts visited
int	ANNptsVisited[20];			// number of pts visited in search
