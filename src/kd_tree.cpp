		...
		ostream &out)					// output stream
{
	...
	if (with_pts) {						// print point coordinates
		...
		for (int i = 0; i < n_pts; i++) {
			out << "\t" << i << ": ";
#if 1 // es code
			m_es_info->get_value_by_index(i);
#endif
	...
	ANNkd_splitter		splitter)		// splitting routine
{
	if (n <= bsp) {						// n small, make a leaf node
		...
		(*splitter)(es_info, pa, pidx, bnd_box, n, dim, cd, cv, n_lo);
	...
	)
{
	...
	annEnclRect(es_info, pa, pidx, n, dd, bnd_box);// construct bounding rectangle
	...
	case ANN_KD_STD:					// standard kd-splitting rule
		root = rkd_tree(es_info, pa, pidx, n, dd, bs, bnd_box, kd_split);
		break;
	case ANN_KD_MIDPT:					// midpoint split
		root = rkd_tree(es_info, pa, pidx, n, dd, bs, bnd_box, midpt_split);
		break;
	case ANN_KD_FAIR:					// fair split
		root = rkd_tree(es_info, pa, pidx, n, dd, bs, bnd_box, fair_split);
		break;
	case ANN_KD_SUGGEST:				// best (in our opinion)
	case ANN_KD_SL_MIDPT:				// sliding midpoint split
		root = rkd_tree(es_info, pa, pidx, n, dd, bs, bnd_box, sl_midpt_split);
		break;
	case ANN_KD_SL_FAIR:				// sliding fair split
		root = rkd_tree(es_info, pa, pidx, n, dd, bs, bnd_box, sl_fair_split);
		break;
	default:
