		...
		ANNdist box_dist)
{
	...
	if (inner_dist <= box_dist) {				// if inner box is closer
		child[ANN_IN]->ann_search(es_info, inner_dist);	// search inner child first
		child[ANN_OUT]->ann_search(es_info, box_dist);	// ...then outer child
	}
	else {										// if outer box is closer
		child[ANN_OUT]->ann_search(es_info, box_dist);	// search outer child first
		child[ANN_IN]->ann_search(es_info, inner_dist);	// ...then outer child
	}
