	...
	int					&n_lo)			// num of points on low side (returned)
{
	...
	cut_dim = annMaxSpread(es_info, pa, pidx, n, dim);
	...
	int					&n_lo)			// num of points on low side (returned)
{
	...
	for (d = 0; d < dim; d++) {
		...
		if (double(bnds.hi[d] - bnds.lo[d]) >= (1-ERR)*max_length) {
										// compute its spread
			ANNcoord spr = annSpread(es_info, pa, pidx, n, d);
	...
	int br1, br2;
	annPlaneSplit(es_info, pa, pidx, n, cut_dim, cut_val, br1, br2);
	//------------------------------------------------------------------
	...
	int					&n_lo)			// num of points on low side (returned)
{
	...
	ANNcoord min, max;
	annMinMax(es_info, pa, pidx, n, cut_dim, min, max);	// find min/max coordinates
	...
	int					&n_lo)			// num of points on low side (returned)
{
	...
	annMinMax(es_info, pa, pidx, n, cut_dim, min, max);
										// is median below lo_cut?
	if (annSplitBalance(pa, pidx, n, cut_dim, lo_cut) >= 0) {
